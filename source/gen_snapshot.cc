  ...
  if (snapshot_kind == kAppAOTAssembly) {
    ...
    if (loading_unit_manifest_filename == nullptr) {
     ...
     else
     {
      ...
      result = Dart_CreateAppAOTSnapshotAsAssembly(&vm_snapshot_data_buffer, &vm_snapshot_data_size,&isolate_snapshot_data_buffer, &isolate_snapshot_data_size,StreamingWriteCallback, file,
                                                   strip, debug_file);
                                                   
      WriteFile("./ios/Flutter/Resource/vm_snapshot_data", vm_snapshot_data_buffer,
                                                                 vm_snapshot_data_size);
      WriteFile("./ios/Flutter/Resource/isolate_snapshot_data", isolate_snapshot_data_buffer,
                                                                 isolate_snapshot_data_size);
