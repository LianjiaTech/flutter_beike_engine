...
is set to release or run \"flutter build ios --release\", then re-run Archive from Xcode."
  ...
  #清理现场
  StreamOutput "clean 现场"
  local thin_resource="${project_path}/ios/Flutter/Resource"
  local thin_resource_zip="${project_path}/ios/Flutter/flutter_resource.zip"
  local thin_uuid_txt="${project_path}/ios/Flutter/uuid_app.txt"

  local  build_product_path="${project_path}/ios/Flutter"
  local  app_framewok_path="${build_product_path}/App.framework"

  echo ${thin_resource_zip}
  if [ -d ${thin_resource} ];then
    rm -rf thin_resource
  fi

  if [ -f ${thin_resource_zip} ];then
    rm -rf thin_resource_zip
  fi
  ...
  #zip resource
  if [ -d ${thin_resource} ];then
    if [ -d "${app_framewok_path}/flutter_assets/NOTICES" ];then
        rm -rf "${app_framewok_path}/flutter_assets/NOTICES"
    fi

     cp -rf "${app_framewok_path}/flutter_assets" ${thin_resource}
     rm -rf "${app_framewok_path}/flutter_assets"

     local  flutter_framewok_path="${build_product_path}/Flutter.framework/icudtl.dat"
     if [ -f ${flutter_framewok_path} ];then
        cp -f "${flutter_framewok_path}" ${thin_resource}
        rm -rf ${flutter_framewok_path}
     fi
     local uuid=`dwarfdump -u --arch=arm64 "${app_framewok_path}/App" | awk -F ' ' '{print $2}'`
     echo ${uuid}>${thin_uuid_txt}
     cd ${build_product_path} &&  zip -r flutter_resource.zip ./Resource
     rm -rf ./Resource
  fi
